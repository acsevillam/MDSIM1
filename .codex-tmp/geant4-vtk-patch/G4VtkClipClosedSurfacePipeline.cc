                   ...
                   vcIn.fViewer->renderer)
{
  ...
  mapper->SetRelativeCoincidentTopologyPolygonOffsetParameters(1, -vc.fDepth);
  ...
  // colour parameters
  if (useVcColour) {
    prop->SetOpacity(vc.alpha);
    prop->SetColor(vc.red, vc.green, vc.blue);
  }
...
void G4VtkClipClosedSurfacePipeline::SetPlane(const G4Plane3D& planeIn)
{
...
void G4VtkClipClosedSurfacePipeline::SetPlane(G4double x, G4double y, G4double z, G4double nx,
                                              G4double ny, G4double nz)
{
...
void G4VtkClipClosedSurfacePipeline::TransformPlane(G4double dx, G4double dy, G4double dz,
