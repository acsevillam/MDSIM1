...
DetectorCylinderMessenger::DetectorCylinderMessenger(DetectorCylinder* detectorCylinder)
    : G4UImessenger(), fDetectorCylinder(detectorCylinder), fCurrentDetectorID(0) {
    fSetCylinderRadiusCmd =
        new G4UIcmdWithADoubleAndUnit("/MultiDetector1/detectors/cylinder/setRadius", this);
    fSetCylinderRadiusCmd->SetGuidance("Set the radius of the cylinder detector.");
    fSetCylinderRadiusCmd->SetParameterName("CylinderRadius", false);
    fSetCylinderRadiusCmd->SetUnitCategory("Length");
    fSetCylinderRadiusCmd->SetRange("CylinderRadius>0.");
    fSetCylinderRadiusCmd->AvailableForStates(G4State_PreInit);

    fSetCylinderHeightCmd =
        new G4UIcmdWithADoubleAndUnit("/MultiDetector1/detectors/cylinder/setHeight", this);
    fSetCylinderHeightCmd->SetGuidance("Set the height of the cylinder detector.");
    fSetCylinderHeightCmd->SetParameterName("CylinderHeight", false);
    fSetCylinderHeightCmd->SetUnitCategory("Length");
    fSetCylinderHeightCmd->SetRange("CylinderHeight>0.");
    fSetCylinderHeightCmd->AvailableForStates(G4State_PreInit);

    fSetCylinderMaterialCmd =
        new G4UIcmdWithAString("/MultiDetector1/detectors/cylinder/setMaterial", this);
    fSetCylinderMaterialCmd->SetGuidance("Set the NIST material name of the cylinder detector.");
    fSetCylinderMaterialCmd->SetParameterName("CylinderMaterial", false);
    fSetCylinderMaterialCmd->AvailableForStates(G4State_PreInit);

    fSetEnvelopeThicknessCmd = new G4UIcmdWithADoubleAndUnit(
        "/MultiDetector1/detectors/cylinder/setEnvelopeThickness", this);
    fSetEnvelopeThicknessCmd->SetGuidance(
        "Set the envelope thickness around the cylinder detector. Zero disables the envelope.");
    ...
    fSetEnvelopeMaterialCmd =
        new G4UIcmdWithAString("/MultiDetector1/detectors/cylinder/setEnvelopeMaterial", this);
    fSetEnvelopeMaterialCmd->SetGuidance("Set the NIST material name of the cylinder envelope.");
    ...
    fSetSplitAtInterfaceCmd =
        new G4UIcmdWithABool("/MultiDetector1/detectors/cylinder/setSplitAtInterface", this);
    fSetSplitAtInterfaceCmd->SetGuidance(
        "Enable automatic split at the WaterBox top interface when the cylinder protrudes into air.");
    ...
    fSetCalibrationFactorCmd =
        new G4UIcmdWithADouble("/MultiDetector1/detectors/cylinder/setCalibrationFactor", this);
    fSetCalibrationFactorCmd->SetGuidance("Set the experimental cylinder calibration factor in cGy/nC.");
    ...
    fSetCalibrationFactorErrorCmd = new G4UIcmdWithADouble(
        "/MultiDetector1/detectors/cylinder/setCalibrationFactorError", this);
    fSetCalibrationFactorErrorCmd->SetGuidance(
        "Set the experimental cylinder calibration factor uncertainty in cGy/nC.");
    ...
    fDetectorIDCmd = new G4UIcmdWithAnInteger("/MultiDetector1/detectors/cylinder/detectorID", this);
    ...
    fTranslateCmd = new G4UIcmdWith3VectorAndUnit("/MultiDetector1/detectors/cylinder/translate", this);
    ...
    fTranslateToCmd =
        new G4UIcmdWith3VectorAndUnit("/MultiDetector1/detectors/cylinder/translateTo", this);
    ...
    fRotateXCmd =
        new G4UIcmdWithADoubleAndUnit("/MultiDetector1/detectors/cylinder/rotateX", this);
    ...
    fRotateYCmd =
        new G4UIcmdWithADoubleAndUnit("/MultiDetector1/detectors/cylinder/rotateY", this);
    ...
    fRotateZCmd =
        new G4UIcmdWithADoubleAndUnit("/MultiDetector1/detectors/cylinder/rotateZ", this);
    ...
    fRotateToCmd = new G4UIcommand("/MultiDetector1/detectors/cylinder/rotateTo", this);
    ...
    fAddGeometryToCmd = new G4UIcommand("/MultiDetector1/detectors/cylinder/addGeometryTo", this);
    ...
    fRemoveGeometryCmd =
        new G4UIcmdWithAnInteger("/MultiDetector1/detectors/cylinder/removeGeometry", this);
...
DetectorCylinderMessenger::~DetectorCylinderMessenger() {
    delete fSetCylinderRadiusCmd;
    delete fSetCylinderHeightCmd;
    delete fSetCylinderMaterialCmd;
...
void DetectorCylinderMessenger::SetNewValue(G4UIcommand* command, G4String newValue) {
    if (command == fSetCylinderRadiusCmd) {
        fDetectorCylinder->SetCylinderRadius(fSetCylinderRadiusCmd->GetNewDoubleValue(newValue));
    } else if (command == fSetCylinderHeightCmd) {
        fDetectorCylinder->SetCylinderHeight(fSetCylinderHeightCmd->GetNewDoubleValue(newValue));
    } else if (command == fSetCylinderMaterialCmd) {
        fDetectorCylinder->SetCylinderMaterial(newValue);
    } else if (command == fSetEnvelopeThicknessCmd) {
        fDetectorCylinder->SetEnvelopeThickness(fSetEnvelopeThicknessCmd->GetNewDoubleValue(newValue));
    } else if (command == fSetEnvelopeMaterialCmd) {
        fDetectorCylinder->SetEnvelopeMaterial(newValue);
    } else if (command == fSetSplitAtInterfaceCmd) {
        fDetectorCylinder->SetSplitAtInterface(fSetSplitAtInterfaceCmd->GetNewBoolValue(newValue));
    } else if (command == fSetCalibrationFactorCmd) {
        fDetectorCylinder->SetCalibrationFactor(
            fSetCalibrationFactorCmd->GetNewDoubleValue(newValue) * (1e-2 * gray) / (1e-9 * coulomb));
    } else if (command == fSetCalibrationFactorErrorCmd) {
        fDetectorCylinder->SetCalibrationFactorError(
            fSetCalibrationFactorErrorCmd->GetNewDoubleValue(newValue) * (1e-2 * gray) / (1e-9 * coulomb));
    } else if (command == fTranslateCmd) {
        fDetectorCylinder->Translate(fCurrentDetectorID, fTranslateCmd->GetNew3VectorValue(newValue));
    } else if (command == fTranslateToCmd) {
        fDetectorCylinder->TranslateTo(fCurrentDetectorID, fTranslateToCmd->GetNew3VectorValue(newValue));
    } else if (command == fRotateXCmd) {
        fDetectorCylinder->RotateX(fCurrentDetectorID, fRotateXCmd->GetNewDoubleValue(newValue));
    } else if (command == fRotateYCmd) {
        fDetectorCylinder->RotateY(fCurrentDetectorID, fRotateYCmd->GetNewDoubleValue(newValue));
    } else if (command == fRotateZCmd) {
        fDetectorCylinder->RotateZ(fCurrentDetectorID, fRotateZCmd->GetNewDoubleValue(newValue));
    } else if (command == fRotateToCmd) {
        G4double theta, phi, psi;
        ConvertToDoubleTrio(newValue, theta, phi, psi);
        fDetectorCylinder->RotateTo(fCurrentDetectorID, theta, phi, psi);
        ...
        DetectorRegistry::GetInstance()->EnableDetector("cylinder");
        fDetectorCylinder->AddGeometryTo(logicalVolumeName, copyNo);
    } else if (command == fRemoveGeometryCmd) {
        fDetectorCylinder->RemoveGeometry(fRemoveGeometryCmd->GetNewIntValue(newValue));
        if (!fDetectorCylinder->HasPlacementRequests()) {
            DetectorRegistry::GetInstance()->DisableDetector("cylinder");
        }
    }
}

void DetectorCylinderMessenger::ConvertToDoubleTrio(const G4String& paramString,
                                                    G4double& xval,
                                                    G4double& yval,
                                                    G4double& zval) {
    G4double x, y, z;
