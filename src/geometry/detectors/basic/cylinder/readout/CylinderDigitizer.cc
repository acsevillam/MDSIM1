...
CylinderDigitizer::CylinderDigitizer(const G4String& name, const CylinderReadoutParameters& readoutParameters)
    : BaseDigitizer(name, "cylinder"),
      fHitsCollection(nullptr),
      fDigitsCollection(nullptr),
      fDCID(-1),
      fReadoutParameters(readoutParameters) {
    collectionName.push_back("CylinderDigitsCollection");
}

CylinderDigitizer::~CylinderDigitizer() = default;

void CylinderDigitizer::Digitize() {
    fDigitsCollection = new CylinderDigitsCollection(moduleName, collectionName[0]);
    auto* digitManager = G4DigiManager::GetDMpointer();
    const G4double sensitiveMass = MD1::GetBasicDosimeterSensitiveVolumeMassOrThrow(
        "DetectorCylinder",
        "CylinderDigitizer::GetSensitiveVolumeMass",
        "CylinderSensitiveVolumeNotFound");

    const G4int hcID = digitManager->GetHitsCollectionID("CylinderHitsCollection");
    if (hcID < 0) {
        StoreDigiCollection(fDigitsCollection);
        return;
    }
    fHitsCollection = static_cast<const CylinderHitsCollection*>(digitManager->GetHitsCollection(hcID));
    ...
    if (fHitsCollection != nullptr) {
        for (size_t i = 0; i < fHitsCollection->GetSize(); ++i) {
            ...
            if (edep > 0.) {
                ...
                auto newDigit = std::make_unique<CylinderDigit>();
