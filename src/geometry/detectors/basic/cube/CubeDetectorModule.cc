...
#include "geometry/detectors/basic/cube/geometry/DetectorCube.hh"
...
#include "geometry/detectors/basic/cube/readout/CubeSensitiveDetector.hh"
...
struct CubeDetectorRuntimeState final : DetectorRuntimeState {
    ...
    MD1::DosimetricDetectorResultsAccumulable resultsAccumulable{"CubeResults"};
...
void ValidateAnalysisIdsOrThrow(G4int signalNtupleId,
    ...
    if (signalNtupleId < 0 || calibrationNtupleId < 0) {
        G4Exception("CubeDetectorModule::ValidateAnalysisIdsOrThrow",
...
const CubeDetectorRuntimeState& GetRuntimeStateOrThrow(const DetectorRuntimeState& runtimeState,
                                                       const G4String& detectorName) {
    return DetectorModuleUtils::GetRuntimeStateOrThrow<CubeDetectorRuntimeState>(
        runtimeState, detectorName, "CubeDetectorModule::GetRuntimeStateOrThrow");
}
...
void CubeDetectorModule::ConstructGeometry(G4LogicalVolume* motherVolume) {
    ...
    if (!CubeDoseCalibrator::HasCalibrationFactor(fGeometry->GetCubeMaterial(),
...
void CubeDetectorModule::PrepareForRun(DetectorRuntimeState& runtimeState, G4bool /*isMaster*/) {
...
void CubeDetectorModule::MergeRunResults(DetectorRuntimeState& /*runtimeState*/, G4bool /*isMaster*/) {}
...
void CubeDetectorModule::CreateAnalysis(G4AnalysisManager* analysisManager,
    ...
    state.signalNtupleId =
        analysisManager->CreateNtuple("CubeHits", "Physical readout variables related to cube detector hits");
    ...
    state.calibrationNtupleId =
        analysisManager->CreateNtuple("CubeDoseCalibration", "Cube calibrated dose output");
...
void CubeDetectorModule::ProcessEvent(const G4Event* event,
    ...
    const CubeDoseCalibrator calibrator(CubeDoseCalibrator::BuildParameters(
        fGeometry->GetCubeMaterial(),
        fGeometry->GetCubeSide(),
        fGeometry->GetEnvelopeMaterial(),
        fGeometry->GetEnvelopeThickness(),
    ...
    for (size_t i = 0; i < digitsCollection->entries(); ++i) {
        ...
        analysisManager->FillNtupleDColumn(state.calibrationNtupleId, 1, digit->GetCollectedCharge());
    ...
    for (const auto& [detectorID, eventTotals] : eventTotalsByDetector) {
        auto& summary =
            state.resultsAccumulable.FindOrCreate(GetSummaryLabel(detectorID));
...
void CubeDetectorModule::PrintResults(const G4Run* /*run*/,
    ...
    MD1::PrintDosimetricDetectorResults(
        "Cube", GetSummaryLabels(), state.resultsAccumulable, context);
}
