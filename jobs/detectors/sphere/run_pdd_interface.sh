...
sweep_description="Depth sweep: z from ${sphere_z_points[0]} cm down to ${sphere_z_points[$((${#sphere_z_points[@]} - 1))]} cm in 0.1 cm steps."
...
printf "replica,sphere_z_cm,estimated_total_absorbed_dose_cGy,mc_err_cGy,mu_err_cGy,det_err_cGy,total_err_cGy,rms_cGy\n" > "${replica_csv}"
...
for ((replica=1; replica<=n_replicas; replica++)); do
    ...
    for sphere_z_cm in "${sphere_z_points[@]}"; do
        point_tag=$(printf "%05.2f" "${sphere_z_cm}" | tr '.' 'p')
        ...
        if [ -z "${estimated_line}" ]; then
            echo "Could not find sphere results line (8) in ${result_dir}/output.log" >&2
            exit 1
        fi
        ...
        if [ -z "${estimated_cGy}" ] || [ -z "${mc_err_cGy}" ] || [ -z "${mu_err_cGy}" ] || [ -z "${det_err_cGy}" ] || [ -z "${total_err_cGy}" ] || [ -z "${rms_cGy}" ]; then
            echo "Could not parse sphere estimated absorbed dose line: ${estimated_line}" >&2
            exit 1
        fi

        printf "%s,%s,%s,%s,%s,%s,%s,%s\n" \
            "${replica}" "${sphere_z_cm}" "${estimated_cGy}" "${mc_err_cGy}" "${mu_err_cGy}" "${det_err_cGy}" "${total_err_cGy}" "${rms_cGy}" \
            >> "${replica_csv}"
    done
done
...
awk -F',' '
    ...
    END {
        printf "sphere_z_cm,depth_cm,mean_estimated_total_absorbed_dose_cGy,stddev_cGy,err_cGy,err_rel_percent,mean_mc_err_cGy,mean_mu_err_cGy,mean_det_err_cGy,mean_total_err_cGy,mean_rms_cGy\n"
